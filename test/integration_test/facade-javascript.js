...
JSrealLoader({
    ...
    JSrealLoader({
        ...
        QUnit.test( "JavaScript EN", function( assert ) {
            ...
            assert.equal(apple,'the apple', 'Utilisation de var');
            assert.equal(love, 'love','Utilisation de var');

            //ADD
            assert.equal(S.add(apple),'the apple', 'add')
            assert.equal(CP(C('and'),NP(D('a'),N('fruit'))).add(apple).add(boy),'a fruit, the apple and the boy', 'add (2 fois)');
            assert.equal(S(VP().add(love).add(apple)).add(boy,0),'The boy loves the apple.', 'add 3 fois + position');
