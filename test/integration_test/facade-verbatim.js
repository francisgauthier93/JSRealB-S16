JSrealLoader({
    ...
    QUnit.test( "Verbatim FR", function( assert ) {
        ...
        assert.equal(S(NP(Pro("je").pe(1)), VP(V("être"), "John")).a("!"), "Je suis John!", "3. Verbatim");
        assert.equal(PP(P("à"), NP(N("midi"))), "à midi", "4. Verbatim");
        assert.equal(VP(V("manger"), "un oeuf"), "mange un oeuf", "5. Verbatim");
        ...
        featureUrl: URL.feature
            ...
            QUnit.test( "Verbatim EN", function( assert ) {
                ...
                assert.equal(S(NP(Pro("I").pe(1)), VP(V("be"), "John")).a("!"), "I am John!", "3. Verbatim");
                assert.equal(S("we did something ", PP(P("at"), NP(N("midnight")))), "We did something at midnight.", "4. Verbatim");
                assert.equal(VP(V("eat"), "an apple"), "eats an apple", "5. Verbatim");
